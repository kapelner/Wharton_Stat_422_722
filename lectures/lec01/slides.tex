...
\item We used a \qu{data-driven approach}. \\
...
\begin{itemize}
\item We believe that $f$ has a very \qu{nice} form. 
\item We value \qu{simplicity} because we wholeheartedly believe in Occam's Razor (simplest theory is the one that should be retained).
...
\begin{block}{Philosophy of creating the fit, $\fhat$}
\begin{itemize}
\item We do not make any assumptions about the form of $f$ \qu{nice} or not. 
\item We have no value for simplicity. We are okay with a world being messy and complex.
\item We believe simple mathematical models such as $V=IR$ and $F = G\frac{m_1 m_2}{d^2}$ work for idealized situations, but never real-life situations with aribitrarily selected predictors measured in arbitrary ways.
\item We would like to know how $f$ works, but it's likely very complex, so it's not our top priority.
...
i.e. $f$ has a assjumed simple form $s$ that has various knobs that can be adjusted based on the data. These knobs are called the \emph{unknown parameters}. Here there are $\ell$ of them $\theta_1, \theta_2, \ldots, \theta_\ell$. It is said the model has $\ell$ \qu{degrees of freedom}. An example of this is the linear model,
...
%\begin{frame}\frametitle{The Linear Model}
%
%Since the conditional mean was linear, the statistical model for regression would be
%
%\beqn
%Y = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p + \errorrv 
%\eeqn
%
%Is this the form you're all familiar with? 
%
%\end{frame}
...
%Because we believe in simple models and we believe in linearity, we assume that
%
%\beqn
%f(x_1, x_2, \ldots, x_p) = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p
%\eeqn
...
What does \qu{minimize $e_1, \ldots, e_n$} mean? We need to define an overall error metric. This is called the \emph{loss function}.
